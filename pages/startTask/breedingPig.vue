<template>
	...
	<view class="mian_background main_content">
		<self-start-task :detail="detail">
			<view slot="task">
				<self-input label="选择任务类型">
					<view slot="fill" class="fill_content">
						<picker @change="taskkindChange" :value="taskkindidx" :range="taskkind">
							<view class="uni-input">{{taskkind[taskkindidx]}}</view>
						</picker>
					</view>
				</self-input>
				<self-input label="选择任务类型">
					<view slot="fill" class="fill_content">
						<picker @change="pigerAdsChange" :value="pigerAdsidx" :range="pigerAds">
							<view class="uni-input">{{pigerAds[pigerAdsidx]}}</view>
						</picker>
					</view>
				</self-input>
				<self-input label="计划时间">
					<view slot="fill" class="fill_content">
						<timer
							fields="second"
			...
			<view slot="car">
				<self-input value="引种车" disabled></self-input>
				<view v-if="accompanying.length" class="otherPeople">
					...
					<view v-for="(item, index) in accompanying" :key="index" class="otherPeople_box" >
						...
						<view class="otherPeople_box_list">
							<text class="otherPeople_box_list_name">进猪口</text>
							<text class="otherPeople_box_list_sfz">{{item.piger}}</text>
						</view>
				...
				<view v-if="accShow">
					...
					<self-input label="进猪口">
						<view slot="fill" class="fill_content">
							<picker @change="pigerInChange" :value="pigerInidx" :range="pigerIn">
								<view class="uni-input">{{pigerIn[pigerInidx]}}</view>
							</picker>
						</view>
					</self-input>
				</view>
