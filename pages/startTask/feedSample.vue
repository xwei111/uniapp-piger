<template>
	...
	<view class="mian_background main_content">
		<self-start-task :detail="detail">
				...
				<self-input label="目标厂家" v-model="targ" placeholder="请输入隔离点"></self-input>
				<self-input label="采样结果">
					<view slot="fill" class="fill_content">
						<picker @change="samplesChange" :value="samplesidx" :range="samples">
							<view class="uni-input">{{samples[samplesidx]}}</view>
						</picker>
					</view>
				</self-input>
				<self-upLoad label="采样照片" @changeHandle="changeHandle"></self-upLoad>
		</self-start-task>
