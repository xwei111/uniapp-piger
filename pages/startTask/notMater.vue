	...
	<view class="mian_background main_content">
		<self-start-task :detail="detail">
			<view slot="task">
				...
				<self-input label="货源公司名称" v-model="comp" ></self-input>
				<self-input label="货源地址" v-model="address" ></self-input>
			</view>
			<view slot="car">
				<self-input label="车辆类别" value="物资运转车" disabled></self-input>
				...
				<!--  -->
				<self-input label="车辆类别" value="铲车" disabled></self-input>
				<view v-if="accompanying1.length" class="otherPeople">
					<view v-for="(item, index) in accompanying1" :key="index" class="otherPeople_box otherPeople_box1" >
						<view class="otherPeople_box_list otherPeople_box_list1">
							<text class="otherPeople_box_list_name">车牌号</text>
							<text class="otherPeople_box_list_sfz">{{item.carIdno}}</text>
						</view>
						<view class="otherPeople_box_delete" @click="deleteHandle1(item, index)">
							<view class="otherPeople_box_delete_icon"></view>
						</view>
					</view>
				</view>
				<view class="addUser" v-if="!accShow1" >
					<text @click="addUserHandle1">+ 新增车辆</text>
				</view>
				<view v-if="accShow1">
					<self-input label="车牌号" v-model="carIdno1" placeholder="请输入车牌号"></self-input>
				</view>
				<view class="addUser" v-if="accShow1"  >
					<text @click="addHandle1">加入该车辆</text>
					<text @click="cancleHandle1">取消</text>
				</view>
			</view>
		</self-start-task>
