<template>
	...
	<view class="mian_background main_content">
		<self-task-box :detail="detail">
			<view slot="task">
				<self-input label="种源地" value="浙A123" disabled></self-input>
			...
			<view class="self_task_basic">车辆洗消</view>
			<self-input label="车牌号" value="浙A 123" disabled></self-input>
			<self-input label="消毒方式" value="熏蒸" disabled></self-input>
			<self-button-radio label="车辆外观是否干净" :dataSource="dataSource2" :active="active2" @selectHandle="selectHandle2"></self-button-radio>
			<self-input label="靠场原因">
				<view slot="fill" class="fill_content">
					<picker @change="arriveWayChange" :value="arriveWayIdx" :range="arriveWays">
						<view class="uni-input">{{arriveWays[arriveWayIdx]}}</view>
					</picker>
			...
			<self-button-radio label="是否采样" :dataSource="dataSource1" :active="active1" @selectHandle="selectHandle1"></self-button-radio>
			<view class="self_task_basic">出猪台信息</view>
			<self-input label="出猪台" value="1号进猪台" disabled></self-input>
